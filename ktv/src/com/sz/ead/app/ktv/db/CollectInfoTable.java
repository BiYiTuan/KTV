/**
 * @Title: CollectInfoTable.java
 * @Prject: Ktv
 * @Package: com.sz.ead.app.ktv.db
 * @Description: 收藏Table
 * @author: zhaoqy
 * @date: 2015-8-6 下午8:30:41
 * @version: V1.0
 */
...
public class CollectInfoTable 
{
	...
	public static final String COLLECT_TABLE = "collecttable";                                            
	public static final Uri CONTENT_COLLECT_URI = Uri.parse("content://" + AUTHORITY + "/" + COLLECT_TABLE); 
	...
	{
		String string = "DROP TABLE IF EXISTS " + COLLECT_TABLE;
		return string;
	}

	/**
	 * @Title: queryCollectVideos
	 * @Description: 查询收藏video
	 * @return
	 * @return: ArrayList<Video>
	 */
	public static ArrayList<Video> queryCollectVideos() 
	{
		...
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
		orderBy = ID + " DESC";  
		cursor = db.query(COLLECT_TABLE, null, where, null, null, null, orderBy);
		if (cursor != null) 
		{
			cursor.setNotificationUri(DatabaseManager.mContext.getContentResolver(), CONTENT_COLLECT_URI);
	...
	/**
	 * 
	 * @Title: queryCollectVideoAmount
	 * @Description: 查询收藏video的数量
	 * @return
	 */
	public static int queryCollectVideoAmount() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return 0;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
		cursor = db.query(COLLECT_TABLE, new String[] {ID}, where, null, null, null, null);
		if (cursor != null) 
		{
	 ...
	 * @return
	 */
	public static Video queryTopRecordCollects() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
	 ...
	 * @return: Video
	 */
	public static Video getOldRecordCollects() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
	 ...
	 * @return
	 */
	public static Video queryCollectsVideo(String rescode) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
		cursor = db.query(COLLECT_TABLE, null, where, null, null, null, null);
		if (cursor != null) 
		{
			int n = cursor.getCount();
			for (int i=0; i<n; i++) 
	...
	public static void insertCollectsVideo(Video video) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
		}
		//要考虑顺序，所以先删除再添加，不update
		deleteCollectsVideo(video.getResCode());
		SQLiteQueryBuilder qb = new SQLiteQueryBuilder();
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
	...
	public static void deleteCollectsVideo(String rescode) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
		db.delete(COLLECT_TABLE, where, null);
	}
	
	public static void deleteCollects() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
	 ...
	 * @return
	 */
	public static boolean isCollectsVideoExist(String rescode) 
	{
		boolean nRet = false;
		if (DatabaseManager.mDbHelper == null) 
		{
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_COLLECT_URI.getPathSegments().get(0));
		mCursor = db.query(COLLECT_TABLE, null, where, null, null, null, null);
