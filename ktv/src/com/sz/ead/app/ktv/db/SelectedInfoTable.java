/**
 * @Title: SelectedInfoTable.java
 * @Prject: Ktv
 * @Package: com.sz.ead.app.ktv.db
 * @Description: 待唱Table
 * @author: zhaoqy
 * @date: 2015-8-6 下午8:53:57
 * @version: V1.0
 */
...
public class SelectedInfoTable 
{
	public static final String AUTHORITY = "com.sz.ead.app.ktv_wakg.db.DatabaseManager";                      
	public static final String SELECTED_TABLE = "selectedtable";                                            
	public static final Uri CONTENT_SELECTED_URI = Uri.parse("content://" + AUTHORITY + "/" + SELECTED_TABLE); 
	...
	{
		String string = "DROP TABLE IF EXISTS " + SELECTED_TABLE;
		return string;
	}

	/**
	 * @Title: querySelectedVideos
	 * @Description: 查询待唱video
	 * @return
	 * @return: ArrayList<Video>
	 */
	public static ArrayList<Video> querySelectedVideos() 
	{
		...
		qb.setTables(CONTENT_SELECTED_URI.getPathSegments().get(0));
		orderBy = ID + " ASC";  
		cursor = db.query(SELECTED_TABLE, null, where, null, null, null, orderBy);
		if (cursor != null) 
		{
			cursor.setNotificationUri(DatabaseManager.mContext.getContentResolver(), CONTENT_SELECTED_URI);
	...
	/**
	 * 
	 * @Title: querySelectedVideosForASC
	 * @Description: 正序查询待唱video
	 * @return
	 * @return: ArrayList<Video>
	 */
	public static ArrayList<Video> querySelectedVideosForASC() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
	...
	/**
	 * 
	 * @Title: querySelectedVideoAmount
	 * @Description: 查询待唱video的数量
	 * @return
	 */
	public static int querySelectedVideoAmount() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return 0;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_SELECTED_URI.getPathSegments().get(0));
		cursor = db.query(SELECTED_TABLE, new String[] {ID}, where, null, null, null, null);
		if (cursor != null) 
		{
	 ...
	 * @return
	 */
	public static Video queryTopRecordSelected() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
	 ...
	 * @return: Video
	 */
	public static Video getOldRecordSelected() 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
	 ...
	 * @return
	 */
	public static Video querySelectedVideo(String rescode) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return null;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_SELECTED_URI.getPathSegments().get(0));
		cursor = db.query(SELECTED_TABLE, null, where, null, null, null, null);
		if (cursor != null) 
		{
			int n = cursor.getCount();
			for (int i=0; i<n; i++) 
	...
	public static void insertSelectedVideo(Video video) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
		...
		db.insert(SELECTED_TABLE, null, values);
	}
	
	public static void insertSelectedVideoForTop(Video video) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
		...
		ContentValues values = new ContentValues();
		values.put(VIDEO_ROWID, video.getRowID());
		values.put(VIDEO_CODE, video.getResCode());
		values.put(VIDEO_NAME, video.getName());
		values.put(VIDEO_TYPE, video.getType());
		values.put(VIDEO_SOURCE, video.getSource());
	...
	public static void insertSelectedVideoList(ArrayList<Video> videoList) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
	...
	public static void insertSelectedVideoListForTop(ArrayList<Video> videoList) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
	...
	public static void deleteSelectedVideoByRowID(String rowid) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
		...
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		qb.setTables(CONTENT_SELECTED_URI.getPathSegments().get(0));
		db.delete(SELECTED_TABLE, where, null);
	}
	
	public static void deleteSelectedVideo(String rescode) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return;
	 ...
	 * @return
	 */
	public static boolean isSelectedVideoExist(String rescode) 
	{
		if (DatabaseManager.mDbHelper == null) 
		{
			return false;
	...
	public static void removeDuplicateSelected() 
	{
		//delete from selectedtable where rescode  in (select  rescode  from people  group  by  rescode   having  count(rescode) > 1)  
		//and _id not in (select min(_id) from  people  group by rescode  having count(videocode )>1)  
		String sql = "delete from " + SELECTED_TABLE + " where " + VIDEO_CODE + " in " + "(select " + VIDEO_CODE + " from " + SELECTED_TABLE + " group by " + 
		             VIDEO_CODE + " having count(" + VIDEO_CODE + ")" + "> 1)" + " and " + ID + " not in " + "(select min(" + ID + ")" + " from " + SELECTED_TABLE +
		             " group by " + VIDEO_CODE + " having count(" + VIDEO_CODE + ")" + "> 1)";	
		Logcat.i(FlagConstant.TAG, "+++++++ removeDuplicateSelected +++++ sql: " + sql);
		SQLiteDatabase db = DatabaseManager.mDbHelper.getReadableDatabase();
		db.execSQL(sql);
	}
}
